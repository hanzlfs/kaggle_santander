# the version of common that allows front-end feature engineer tests

import hashlib, csv, math, os, pickle, subprocess

FEAT_CAT = ['nomprov', 'conyuemp', 'segmento', 'indrel_1mes', 'indfall', \
...
ALL_PROVS = ['UNKNOWN', 'ALAVA', 'ALBACETE', 'ALICANTE', 'ALMERIA', 'ASTURIAS',
...
month_list = ['2015-01-28', '2015-02-28', '2015-03-28', '2015-04-28', '2015-05-28', '2015-06-28',\
                    '2015-07-28', '2015-08-28', '2015-09-28', '2015-10-28', '2015-11-28', '2015-12-28', '2016-01-28',\
...
def getRent_bin(val):
    missing_value = 101402.55 # median
    rent = str(val).strip()
    if rent == 'NA' or rent == '':
        rent = missing_value
    if rent >= 175928.4:
...
def getCustSeniority_bin(val):
    missing_value = 0 # minimum
    cust_seniority = str(val).strip()
    if cust_seniority == 'NA' or cust_seniority == '':
        cust_seniority = missing_value
    if cust_seniority >= 155.0:
        ibin = 4
    elif cust_seniority >= 87.0:
        ibin = 3
    elif cust_seniority >= 39.0:
        ibin = 2
    elif cust_seniority >= 18.0:
        ibin = 1
    else:
        ibin = 0
    return ibin
...
def getAge(row):
    ...
    if age == 'NA' or age == '':
        ...
        if age < min_age:
            ...
            age = max_age
    val = round( (age - min_age) / range_age, 4)
    #return [val] + feat
    return val
    #return age
...
def getCustSeniority(row):
    ...
    val = round((cust_seniority-min_value) / range_value, 4)
    #return  [val] + feat
    return val
...
def getRent(row):
    ...
    if rent == 'NA' or rent == '':
        ...
        if rent < min_value:
            ...
            rent = max_value
    val = round((rent-min_value) / range_value, 6)
    #return  [val] + feat
    return val
    #return rent
    #return round((rent-min_value) / range_value, 6)
    
def getRent_keep_missing(row):
    rent = row['renta'].strip()
    if rent == 'NA' or rent == '':
        rent = -99
    else:
...
if __name__ == "__main__":
    print len(TARGET) + len(FEAT_CAT) + len(FEAT_NUM)